"""
Build AtomPair NN search data files.

Reads filtered_df.parquet and produces:
    nn_data/
        atompair_packed.npy             ← packed-bit AtomPair matrix
        atompair_smiles.npy             ← parallel SMILES array
        active_atompair_fp_indices.npy  ← active meta-row -> fp matrix row
        inactive_atompair_fp_indices.npy

Reuses existing active_meta.parquet, inactive_meta.parquet,
active_target_index.pkl, inactive_target_index.pkl.

Usage:
    python build_atompair_nn.py
"""
...
OUTPUT_DIR = "nn_data"
FP_PACKED_PATH = os.path.join(OUTPUT_DIR, "atompair_packed.npy")
FP_SMILES_PATH = os.path.join(OUTPUT_DIR, "atompair_smiles.npy")
FP_BITS = 4096
...
# ---------------------------------------------------------------------------
# Compute AtomPair fingerprints (matches training-time calculation exactly)
# ---------------------------------------------------------------------------
def smi_to_packed_atompair(smi):
...
def compute_fingerprints():
    print(f"\nComputing AtomPair fingerprints for {len(unique_smiles):,} compounds...")
    smiles_list = []
    fp_list = []
    for smi in tqdm(unique_smiles):
        fp = smi_to_packed_atompair(smi)
        if fp is not None:
...
# ---------------------------------------------------------------------------
# Build per-class fp_indices (which compounds parsed with AtomPair)
# ---------------------------------------------------------------------------
smi_to_idx = {s: i for i, s in enumerate(smiles_arr)}

def build_fp_indices(activity_label, basename):
    print(f"\nBuilding {basename}_atompair_fp_indices ...")

    # Read the existing meta file (shared across fingerprints)
    ...
    # Map each meta row to a row in the AtomPair fingerprint matrix
    ...
    if n_missing:
        print(f"  WARNING: {n_missing:,} interactions reference SMILES with no AtomPair fingerprint")
        print(f"  These interactions will be excluded from AtomPair NN search.")

    fp_idx_path = os.path.join(OUTPUT_DIR, f"{basename}_atompair_fp_indices.npy")
